...
def find_closest_sipm(given_pos, df):
    ### Find the closest SiPM to the true average point
    sipms = DataSiPM_idx.loc[df.sensor_id]
    positions = np.array([sipms.X, sipms.Y, sipms.Z]).transpose()
    subtr = np.subtract(given_pos, positions)
...
def assign_sipms_to_gammas(waveforms, true_pos, DataSiPM_idx):
    ...
    sns_closest_pos = [np.array([find_closest_sipm(pos, waveforms).X.values, find_closest_sipm(pos, waveforms).Y.values, \
                                 find_closest_sipm(pos, waveforms).Z.values]).transpose()[0] for pos in true_pos]

    q1, q2     = [], []
    pos1, pos2 = [], []
    
    closest_pos = sns_closest_pos[0] ## I keep the first one, which always exists.
